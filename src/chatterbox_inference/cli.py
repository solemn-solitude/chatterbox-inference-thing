...
@run.command()
@click.option("--host", default=None, help="Host to bind to (default: from env or *)")
@click.option(
    "--port", default=None, type=int, help="Port to bind to (default: from env or 5555)"
)
@click.option("--log-level", default=None, help="Log level (default: from env or INFO)")
@click.option(
    "--offload-timeout",
...
def generate(base_voice_id, templates, skip_existing, log_level, keep_warm):
    ...
    Example:
        chatterbox-inference anchors generate my_voice
        chatterbox-inference anchors generate my_voice --templates excited_01,sad_01
    """
    setup_logging(log_level)
    logger = logging.getLogger(__name__)
...
def analyze(base_voice_id, anchor_id, update_db, log_level):
    ...
    Example:
        chatterbox-inference anchors analyze my_voice
        chatterbox-inference anchors analyze my_voice --anchor-id my_voice_excited_01
        chatterbox-inference anchors analyze my_voice --update-db
    """
    setup_logging(log_level)
    logger = logging.getLogger(__name__)
...
def config_info():
    ...
    click.echo(
        dedent(f"""
        Chatterbox Inference Configuration
        ========================================
